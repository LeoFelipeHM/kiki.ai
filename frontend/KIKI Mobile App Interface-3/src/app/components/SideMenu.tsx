import { Calendar, MessageCircle, X, Settings, Sparkles, Home, FileText, Bot } from 'lucide-react';
import { useTheme } from './ThemeProvider';

interface SideMenuProps {
  isOpen: boolean;
  onClose: () => void;
  onNavigate: (screen: 'home' | 'chat' | 'calendar' | 'profile' | 'settings' | 'notes' | 'agents') => void;
...
export function SideMenu({ isOpen, onClose, onNavigate, currentScreen, userName = 'Maria Silva', userEmail = 'maria.silva@email.com' }: SideMenuProps) {
  ...
  const menuItems = [
    ...
    { id: 'agents' as const, icon: Bot, label: 'Agentes', description: 'Assistentes autônomos' },
  ...
  const handleNavigate = (screen: 'home' | 'chat' | 'calendar' | 'profile' | 'settings' | 'notes' | 'agents') => {
    onNavigate(screen);
    onClose();
  };
